  ...
  test 'should see the author name when the post was written by current user' do
    ...
    assert_select 'ul.posts' do
      assert_select 'li.post span.author',
                                    text: "#{@other_post.user.name}", count: 0
      assert_select 'li.post span.author', text: "#{@post.user.name}", count: 0
    end
    log_in_as @post.user
    assert is_logged_in?
    get new_post_path
    ...
    follow_redirect!
    assert_select 'ul.posts' do
      assert_select 'li.post span.author', text: "#{@post.user.name}", count: 1
    end
    delete logout_path
    ...
    assert_select 'ul.posts' do
      assert_select 'li.post span.author',
                                    text: "#{@other_post.user.name}", count: 1
      assert_select 'li.post span.author', text: "#{@post.user.name}", count: 0
    end
  end
end
